drop table if exists Marca cascade;

drop table if exists Entidade cascade;

drop table if exists Treinador cascade;

drop table if exists ComissaoTecnica cascade;

drop table if exists AuxiliarTecnico cascade;

drop table if exists Clube cascade;

drop table if exists Campeonato cascade;

drop table if exists ClubeCampeonato cascade;

drop table if exists Diretor cascade;

drop table if exists Estadio cascade;

drop table if exists Arbitro cascade;

drop table if exists ArbitroAuxiliar cascade;

drop table if exists Partida cascade;

drop table if exists PartidaArbitroAuxiliar cascade;

drop table if exists Posicao cascade;

drop table if exists Jogador cascade;

drop table if exists JogadorPosicao cascade;

drop table if exists Atuacao cascade;

drop table if exists Falta cascade;

drop table if exists Cartao cascade;

drop table if exists Gol cascade;